...
class PhpSerial
{
    ...
    /**
     * port reading with response expected in a period of time.
     *
     * @param string  $response     response expected
     * @param int     $timeout      Max time execution
     * @return bool
     */
    public function readTimeout($response,$timeout=1){
      $start_time = time() ;
      while(true) {
        ...
        if (strpos($this->readPort(), $response) !== FALSE) {
          return true ;
          break;
        }
        else {
    ...
    /**
     * write / read port with response expected in a period of time.
     *
     * @param string  $query        string to be sent to the device
     * @param string  $response      response expected
     * @param int     $timeout      Max time execution
     * @return bool
     */
    public function writeReadTimeout($query,$response,$timeout=1){
      $this->sendMessage($query);
      return $this->readTimeout($response,$timeout) ;
    }
